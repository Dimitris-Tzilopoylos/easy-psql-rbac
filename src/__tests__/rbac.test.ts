...
function resetRegistry() {
  (DB as any).models = {
    public: {
      ...
      users_aggregate: Object.assign(new MockUserModel(), {
        isAggregate: true,
      }),
      posts_aggregate: Object.assign(new MockPostModel(), {
        isAggregate: true,
      }),
    },
  };
...
describe("access control gates", () => {
  ...
  it("throws ForbiddenError when role has no permissions for the requested table", () => {
    rbac.withRole("no-table", (r) =>
      r.findMany("public", "other", { columns: ["id"] }),
    );
    expect(() =>
      rbac.findManyModel({
        schema: "public",
  ...
  it("throws ForbiddenError when entity permissions have an empty columns array", () => {
    rbac.withRole("empty-cols", (r) =>
      r.findMany("public", "users", { columns: [] }),
    );
    expect(() =>
      rbac.findManyModel({
        schema: "public",
...
describe("insert sanitization", () => {
  ...
  it("relation key in body recurses and throws when nested body has a disallowed column", () => {
    ...
    const body: any = {
      title: "Hello",
      user_id: 1,
      user: { name: "Alice", email: "a@b.com", secret: "x" },
    };
    expect(() =>
      rbac.createOneModel({
        schema: "public",
  ...
  it("throws ForbiddenError when update body contains a disallowed column", () => {
    expect(() =>
      rbac.updateOneModel({
        ...
        table: "users",
        user: BASE_USER,
        input: { update: { id: 99, name: "Bob" }, where: {} },
      }),
    ).toThrow(ForbiddenError);
  });
...
describe("where sanitization", () => {
  let rbac: EasyPSQLRBAC;
  const usersModel = () =>
    (DB as any).models["public"]["users"] as MockUserModel;
  ...
  it("throws ForbiddenError when where contains a model column not in the allowed list", () => {
    expect(() =>
      rbac.roleBasedWhereSanitization({
        ...
        apiAccessType: AllowedEngineApiAccessTypes.findMany,
        user: BASE_USER,
        entityPermissions: EP.usersRestricted,
      }),
    ).toThrow(ForbiddenError);
  });
  ...
  it("recurses into _and arrays", () => {
    expect(() =>
      rbac.roleBasedWhereSanitization({
        model: usersModel() as any,
        where: { _and: [{ name: { _eq: "A" } }, { id: { _eq: 1 } }] },
        apiAccessType: AllowedEngineApiAccessTypes.findMany,
        user: BASE_USER,
        entityPermissions: fullPerms,
      }),
...
describe("mergeWhereWithPreConditions", () => {
  ...
  it("does not crash when preConditions.where has _or but where has none yet", () => {
    const result = rbac.mergeWhereWithPreConditions({
      where: { id: 1 },
      preConditions: { where: { _or: [{ status: "a" }] } },
    });
...
describe("access type enforcement", () => {
  ...
  it("findOneModel uses findOne — not findMany", () => {
    ...
    expect(() =>
      rbac.findOneModel({ schema: "public", table: "users", user, query: {} }),
    ).not.toThrow();
    expect(() =>
  ...
  it("updateOneModel uses updateOne — not updateMany", () => {
    ...
    expect(() =>
      rbac.updateOneModel({
        ...
        table: "users",
        user,
        input: { update: { name: "Bob" }, where: {} },
      }),
    ).not.toThrow();
    expect(() =>
      rbac.updateManyModel({
        schema: "public",
        table: "users",
        user,
        input: { update: { name: "Bob" }, where: {} },
  ...
  it("deleteOneModel uses deleteOne — not deleteMany", () => {
    ...
    expect(() =>
      rbac.deleteOneModel({
        ...
        table: "users",
        user,
        input: { where: { id: { _eq: 1 } } },
      }),
    ).not.toThrow();
    expect(() =>
      rbac.deleteManyModel({
        schema: "public",
        table: "users",
        user,
        input: { where: { id: { _eq: 1 } } },
...
describe("RoleRegistry", () => {
  ...
  it("addPermission initialises missing schema and table entries", () => {
    ...
    expect(
      role.options.permissions!.entities!["app"]["orders"]["findMany"],
    ).toEqual({ columns: ["order_id"] });
  });
});
...
describe("registry isolation", () => {
  it("preConditions.where spread into input.where does not share a reference with the registry", () => {
    const rbac = new EasyPSQLRBAC();
    rbac.withRole("r", (r) =>
      r.findMany("public", "users", {
        columns: ["id", "name", "email", "user_id"],
        preConditions: { where: { status: "active" } },
      }),
    );
    ...
    // Mutate the merged where
    query.where.status = "HACKED";

    // Registry must be unchanged
    const role = rbac.findRoleById("r")!;
    const stored = role.options.permissions!.entities!["public"]["users"][
      "findMany"
    ] as any;
    expect(stored.preConditions.where.status).toBe("active");
  });
});
...
describe("input validation guards", () => {
  ...
  it("throws BadRequest when findManyModel is called with a non-object query", () => {
    expect(() =>
      rbac.findManyModel({
        schema: "public",
        table: "users",
  ...
  it("throws BadRequest when createOneModel body is null", () => {
    expect(() =>
      rbac.createOneModel({
        schema: "public",
        table: "users",
  ...
  it("throws BadRequest when createManyModel body contains a non-object row", () => {
    expect(() =>
      rbac.createManyModel({
        schema: "public",
        table: "users",
        user: BASE_USER,
        body: [{ name: "Alice", email: "a@b.com" }, null],
      }),
    ).toThrow(BadRequest);
  });

  it("throws BadRequest when updateOneModel input.update is not an object", () => {
    expect(() =>
      rbac.updateOneModel({
        schema: "public",
        table: "users",
        user: BASE_USER,
        input: { update: null, where: {} },
      }),
    ).toThrow(BadRequest);
  });

  it("null and undefined include values are normalised to {} and processed without crashing", () => {
    const query: any = { include: { user: null } };
    // posts.findMany allows user_id so the join from_column is in select — but user relation on users model doesn't exist
    // so this should throw ForbiddenError (no relation named 'user' on users table), not crash
    expect(() =>
      rbac.findManyModel({
        schema: "public",
        table: "users",
